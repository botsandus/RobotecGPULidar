...
{
    std::vector<TestPointStructWithoutIsHit> points = generateTestPointsArrayWithoutIsHit(100);

    EXPECT_RGL_SUCCESS(rgl_node_points_from_array(&usePointsNode, points.data(), points.size(), pointFieldsWithoutIsHit.data(), pointFieldsWithoutIsHit.size()));
    EXPECT_RGL_SUCCESS(rgl_node_points_compact(&compactNode));
